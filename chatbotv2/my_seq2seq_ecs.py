...
tf.app.flags.DEFINE_string("input_file", "corpus.segment.pair", "input data file")
...
def load_word_set(input_file):
    with tf.gfile.GFile(input_file, 'r') as file_object:
        while True:
...
class MySeq2Seq(object):
    ...
    def generate_trainig_data(self):
        ...
        vector_file = os.path.join(FLAGS.buckets, FLAGS.vector_file)

        load_word_set(input_file)
        load_vectors(vector_file)
        init_seq(input_file)
        xy_data = []
        y_data = []
        print "question_seqs=", len(question_seqs)
    ...
    def train(self):
        ...
        model_file = os.path.join(FLAGS.checkpointDir, "model")
...
if __name__ == '__main__':
    my_seq2seq = MySeq2Seq(max_seq_len=FLAGS.max_seq_len, word_vec_dim=FLAGS.word_vec_dim)
